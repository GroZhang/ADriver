...
public:
	...
	void SetDefaultValue(float feedbackValue)
	{
		...
		debugPrintf(L"ASIOUAC: Set default value: %f\n", interval * feedbackValue);
#endif
		m_guard.Leave();
	}
...
template <class TaskClass> class BaseThread
{
	...
	void ThreadFunc()
	{
		bool retVal = TRUE;
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: %s. Thread started!\n", m_Task.TaskName());
#endif
...
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: %s. Thread exited!\n", m_Task.TaskName());
#endif
		ExitThread(0);
	}
protected:
...
public:
	...
	{
#ifdef _ENABLE_TRACE
			debugPrintf(L"ASIOUAC: %s. Thread constructor\n", m_Task.TaskName());
#endif
...
#ifdef _ENABLE_TRACE
			debugPrintf(L"ASIOUAC: %s. Thread destructor\n", m_Task.TaskName());
#endif
		}
	}
	...
	{
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: %s. Try start...\n", m_Task.TaskName());
#endif
		if(m_Thread == INVALID_HANDLE_VALUE)
		{
#ifdef _ENABLE_TRACE
			debugPrintf(L"ASIOUAC: %s. Can't start thread. Invalid handle\n", m_Task.TaskName());
#endif
			return FALSE;
		}
		if(m_taskState == TaskThread::TaskStarted)
		{
#ifdef _ENABLE_TRACE
			debugPrintf(L"ASIOUAC: %s. Can't start thread. Already started\n", m_Task.TaskName());
#endif
			return TRUE;
		}
...
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: %s. Start is OK\n", m_Task.TaskName());
#endif
		return retVal;
	}
	...
	{
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: %s. Try stop...\n", m_Task.TaskName());
#endif
		if(m_Thread == INVALID_HANDLE_VALUE)
		{
#ifdef _ENABLE_TRACE
			debugPrintf(L"ASIOUAC: %s. Can't stop thread. Invalid handle\n", m_Task.TaskName());
#endif
			return FALSE;
		}
		...
		{
#ifdef _ENABLE_TRACE
			debugPrintf(L"ASIOUAC: %s. Can't stop thread. Already stopped\n", m_Task.TaskName());
#endif
			return TRUE;
		}
...
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: %s. Wait current job ending ...\n", m_Task.TaskName());
#endif
		//wait current job ending
		m_inWork.Enter();
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: %s. Wait current job ending successfully\n", m_Task.TaskName());
#endif
		...
		m_inWork.Leave();
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: %s.Stop is OK\n", m_Task.TaskName());
#endif
		return retVal;
	}
...
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: %s. AudioTask::Init(MaxPacketSize=%d, Interval=%d, ChannelNum=%d, SampleSize=%d)\n", 
...
public:
	AudioDACTask() : AudioTask(packetPerTransferDAC, L"Audio DAC task"), m_feedbackInfo(NULL), m_readDataCb(NULL), m_readDataCbContext(NULL)
	{
		...
		debugPrintf(L"Created file c:\\dac_dump.bin");
#endif
	}
