...
USBDevice::USBDevice() : m_usbDeviceHandle(NULL), m_deviceInfo(NULL), m_errorCode(ERROR_SUCCESS), m_deviceSpeed(HighSpeed), m_deviceMutex(NULL), m_deviceIsConnected(FALSE)
{
	InitDescriptors();
#ifdef _ENABLE_TRACE
	debugPrintf(L"ASIOUAC: USBDevice()\n");
#endif
}
...
#ifdef _ENABLE_TRACE
	debugPrintf(L"ASIOUAC: UsbK_ControlTransfer failed. ErrorCode: %08Xh\n",  m_errorCode);
#endif
	return FALSE;
}
...
KUSB_HANDLE USBDevice::FindDevice()
{
	...
	{
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: Error initializing device list.\n");
#endif
		return NULL;
	}
	...
	{
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: No devices in device list.\n");
#endif
	...
	{
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: Device not found.\n");
#endif
		// If LstK_Init returns TRUE, the list must be freed.
		LstK_Free(DeviceList);
		return NULL;
    ...
    if (!UsbK_Init(&handle, m_deviceInfo))
    {
        ...
        m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
        debugPrintf(L"ASIOUAC: UsbK_Init failed. ErrorCode: %08Xh\n",  m_errorCode);
#endif
    }
...
bool USBDevice::InitDevice()
{
	...
	m_deviceMutex = CreateMutex(NULL, FALSE, L"Global\\ASIOUAC2");
	if(GetLastErrorInternal() == ERROR_ALREADY_EXISTS)
	{
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: Can't start device! Device already used!\n");
#endif
		if(m_deviceMutex)
		{
			CloseHandle(m_deviceMutex);
	...
	if(!UsbK_QueryDeviceInformation( m_usbDeviceHandle, DEVICE_SPEED, &lengthTransferred, configDescriptorBuffer))
	{
		m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
        debugPrintf(L"ASIOUAC: UsbK_QueryDeviceInformation failed. ErrorCode: %08Xh\n",  m_errorCode);
#endif
	    UsbK_Free(m_usbDeviceHandle);
	...
	if(m_deviceSpeed == HighSpeed)
        debugPrintf(L"ASIOUAC: Device speed: high\n");
	else
        debugPrintf(L"ASIOUAC: Device speed: low/full\n");
#endif
	if(!UsbK_GetDescriptor(m_usbDeviceHandle, USB_DESCRIPTOR_TYPE_DEVICE, 0, 0, configDescriptorBuffer, sizeof(configDescriptorBuffer), &lengthTransferred))
	{
        m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
        debugPrintf(L"ASIOUAC: Get device descriptor failed. ErrorCode: %08Xh\n",  m_errorCode);
#endif
	    UsbK_Free(m_usbDeviceHandle);
		m_usbDeviceHandle = NULL;
		return FALSE;
	...
	else
	{
        m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
        debugPrintf(L"ASIOUAC: Get config descriptor failed. ErrorCode: %08Xh\n",  m_errorCode);
#endif
	    UsbK_Free(m_usbDeviceHandle);
		m_usbDeviceHandle = NULL;
		return FALSE;
