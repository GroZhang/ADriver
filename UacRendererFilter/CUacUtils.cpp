...
HRESULT CUacUtils::GetDeviceInfos(bool includeDisconnected, UacDeviceInfo** ppDestInfos, int* pInfoCount, int* pIndexDefault)
{
	HRESULT hr=S_OK;
	...
	*ppDestInfos=NULL;
	...
	if (FAILED(hr))
	{			
		DebugPrintf(L"CUacUtils::GetDeviceInfos - Unable to instantiate device enumerator: %x\n", hr);
		goto exit;
	}
	...
	if (FAILED(hr))
	{			
		DebugPrintf(L"CUacUtils::GetDeviceInfos - Unable to enumerate endpoints. %x\n", hr);
		goto exit;
	}
	...
	if (FAILED(hr))
	{			
		DebugPrintf(L"CUacUtils::GetDeviceInfos - Unable to get endpoint count. %x\n", hr);
		goto exit;
	}
	...
	*pInfoCount=endpointCount;
	...
	if (endpointCount == 0)
    {
        DebugPrintf(L"CUacUtils::GetDeviceInfos - No endpoints found.\n");
		return S_FALSE;
    }
	...
	*ppDestInfos=new UacDeviceInfo[endpointCount];
	UacDeviceInfo* pDestInfos=*ppDestInfos;
