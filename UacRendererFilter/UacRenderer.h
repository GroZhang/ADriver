...
{
public:
	CUacRenderer(LPCWSTR pDevID);
    ~CUacRenderer(void);

	WAVEFORMATEX* GetUacMixFormat();

	//BEGIN: AudioDevice
	bool CheckFormat(WAVEFORMATEX* requestedFormat, WAVEFORMATEX** suggestedFormat, AUDCLNT_SHAREMODE shareMode);
	bool CheckFormatExclusive(WAVEFORMATEX* requestedFormat, WAVEFORMATEX** ppSuggestedFormat);
	//END: AudioDevice
...
private:
    ...
    static DWORD __stdcall UacRenderThread(LPVOID Context);
    DWORD CUacRenderer::DoRenderThread();

	ASIOBufferInfo* buffer_info = NULL;
};
