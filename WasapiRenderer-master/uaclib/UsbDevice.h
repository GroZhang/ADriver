...
#ifdef _ENABLE_TRACE
//extern void debugPrintf(Lconst _TCHAR *szFormat, ...);
extern void debugPrintf(const wchar_t *str, ...);
...
protected:
	...
	void CheckError(int currentError)
	{
		...
		{
#ifdef _ENABLE_TRACE
	        debugPrintf(L"ASIOUAC: Found critical error with ErrorCode: %08Xh\n",  currentError);
#endif
...
public:
...
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: OvlK_Init failed. ErrorCode: %08Xh\n", m_errorCode);
#endif
		return FALSE;
	}
    ...
    bool OvlAcquire(KOVL_HANDLE* OverlappedK, KOVL_POOL_HANDLE PoolHandle)
	{
		...
		m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: OvlK_Acquire failed. ErrorCode: %08Xh\n", m_errorCode);
#endif
		return FALSE;
	}
	...
	bool OvlWait(KOVL_HANDLE OverlappedK, LONG TimeoutMS, KOVL_WAIT_FLAG WaitFlags, PUINT TransferredLength)
	{
		...
		m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: OvlK_Wait failed. ErrorCode: %08Xh\n", m_errorCode);
#endif
		return FALSE;
	}
    ...
    bool OvlWaitOrCancel(KOVL_HANDLE OverlappedK, LONG TimeoutMS, PUINT TransferredLength)
	{
		...
		if(TimeoutMS != 0) // we are just cancel request, no errors report
		{
			m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
			debugPrintf(L"ASIOUAC: OvlK_WaitOrCancel failed. ErrorCode: %08Xh\n", m_errorCode);
#endif
		}
		return FALSE;
	}
	...
	bool OvlRelease(KOVL_HANDLE OverlappedK)
	{
		...
		m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: OvlK_Release failed. ErrorCode: %08Xh\n", m_errorCode);
#endif
		return FALSE;
	}
	...
	bool OvlReUse(KOVL_HANDLE OverlappedK)
	{
		...
		m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: OvlK_ReUse failed. ErrorCode: %08Xh\n", m_errorCode);
#endif
		return FALSE;
	}
    ...
    bool UsbResetPipe(UCHAR PipeId)
	{
		...
		m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: UsbK_ResetPipe failed. ErrorCode: %08Xh\n", m_errorCode);
#endif
		return FALSE;
	}
    ...
    bool UsbAbortPipe(UCHAR PipeId)
	{
		...
		m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: UsbK_AbortPipe failed. ErrorCode: %08Xh\n", m_errorCode);
#endif
		return FALSE;
	}
...
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: UsbK_IsoWritePipe failed. ErrorCode: %08Xh\n", m_errorCode);
#endif
		return FALSE;
	}
	...
	bool UsbIsoReadPipe (UCHAR PipeID, PUCHAR Buffer, ULONG BufferLength, LPOVERLAPPED Overlapped, PKISO_CONTEXT IsoContext)
	{
		...
		m_errorCode = lastError;
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: UsbK_IsoReadPipe failed. ErrorCode: %08Xh\n", m_errorCode);
#endif
		return FALSE;
	}
	...
	bool UsbClaimInterface(UCHAR Number)
	{
		...
		m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: UsbK_ClaimInterface %d failed. ErrorCode: %08Xh\n", Number, m_errorCode);
#endif
		return FALSE;
	}
	...
	bool UsbSetAltInterface(UCHAR Number, UCHAR AltSettingNumber)
	{
		...
		m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: UsbK_SetAltInterface %d (alt %d) failed. ErrorCode: %08Xh\n", Number, AltSettingNumber, m_errorCode);
#endif
		return FALSE;
	}
	...
	bool UsbReleaseInterface(UCHAR Number)
	{
		...
		m_errorCode = GetLastErrorInternal();
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: UsbK_ReleaseInterface %d failed. ErrorCode: %08Xh\n", Number, m_errorCode);
#endif
		return FALSE;
	}
