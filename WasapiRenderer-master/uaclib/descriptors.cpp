...
#ifdef _ENABLE_TRACE
		debugPrintf(L"ASIOUAC: Found CS descriptor for endpoint 0x%X, audio streaming interface 0x%X\n", m_descriptor.bEndpointAddress, m_interface->Descriptor().bInterfaceNumber);
#endif
		return TRUE;
	}
	return FALSE;
...
#ifdef _ENABLE_TRACE
			debugPrintf(L"ASIOUAC: Found CS descriptor (subtype 0x%X) for audio streaming interface 0x%X\n", m_interface.bInterfaceSubClass, m_interface.bInterfaceNumber);
#endif
			return TRUE;
		case FORMAT_SUB_TYPE:			
			memcpy(&m_formatDescriptor, csDescriptor, sizeof(usb_format_type_2));
#ifdef _ENABLE_TRACE
			debugPrintf(L"ASIOUAC: Found format type descriptor for audio streaming interface 0x%X\n", m_interface.bInterfaceNumber);
#endif
			return TRUE;
	}
	return FALSE;
...
#ifdef _ENABLE_TRACE
	debugPrintf(L"ASIOUAC: Found endpoint 0x%X for audio streaming interface 0x%X\n", ep->m_descriptor.bEndpointAddress, m_interface.bInterfaceNumber);
#endif
