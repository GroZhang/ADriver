...
// Local Functions 
// ******************************************************************
static LONG findDrvPath(WCHAR *clsidstr, WCHAR *dllpath, int dllpathsize)
{
	...
	BOOL			found = FALSE;
	char			path[MAX_PATH];
	CharLowerBuff(clsidstr,lstrlenW(clsidstr));
	if ((cr = RegOpenKey(HKEY_CLASSES_ROOT,COM_CLSID,&hkEnum)) == ERROR_SUCCESS) {
		...
		while (cr == ERROR_SUCCESS && !found) {
			...
			if (cr == ERROR_SUCCESS) {
				CharLowerBuff(databuf,lstrlenW(databuf));
				if (!(lstrcmpW(databuf,clsidstr))) {
					if ((cr = RegOpenKeyEx(hkEnum,(LPCTSTR)databuf,0,KEY_READ,&hksub)) == ERROR_SUCCESS) {
						if ((cr = RegOpenKeyEx(hksub,(LPCTSTR)INPROC_SERVER,0,KEY_READ,&hkpath)) == ERROR_SUCCESS) {
							...
							if (cr == ERROR_SUCCESS) {
								...
								wcstombs(path, dllpath, lstrlen(dllpath));
								hfile = OpenFile(path, &ofs, OF_EXIST);
								if (hfile) rc = 0; 
							}
							RegCloseKey(hkpath);
						}
...
static LPASIODRVSTRUCT newDrvStruct(HKEY hkey, WCHAR *keyname, int drvID, LPASIODRVSTRUCT lpdrv)
{
	HKEY	hksub;
	WCHAR	databuf[256];
			...
			if (cr == ERROR_SUCCESS) {
				...
				if (rc == 0) {
					...
					if (lpdrv) {
						...
						if (cr == ERROR_SUCCESS) {
							lstrcpy(lpdrv->drvname,databuf);
						}
						else lstrcpy(lpdrv->drvname,keyname);
					}
				}
			}
			RegCloseKey(hksub);
...
LONG AsioDriverList::asioGetDriverName(int drvID, WCHAR *drvname, int drvnamesize)
{	
	LPASIODRVSTRUCT			lpdrv = 0;
	...
	if ((lpdrv = getDrvStruct(drvID,lpdrvlist)) != 0) {
		if (lstrlen(lpdrv->drvname) < (unsigned int)drvnamesize) {
			lstrcpy(drvname,lpdrv->drvname);
		}
		else {
...
LONG AsioDriverList::asioGetDriverPath(int drvID, WCHAR *dllpath, int dllpathsize)
{
	LPASIODRVSTRUCT			lpdrv = 0;
	...
	if ((lpdrv = getDrvStruct(drvID,lpdrvlist)) != 0) {
		if (lstrlen(lpdrv->dllpath) < (unsigned int)dllpathsize) {
			lstrcpy(dllpath,lpdrv->dllpath);
			return 0;
		}
		dllpath[0] = 0;
