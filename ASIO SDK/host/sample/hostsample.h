...
{	// the actual processing callback.
	...
	WCHAR tmp[128];
	wsprintf (tmp, _T("diff: %d / %d ms / %d ms / %d samples                 \n"), asioDriverInfo.sysRefTime - (long)(asioDriverInfo.nanoSeconds / 1000000.0), asioDriverInfo.sysRefTime, (long)(asioDriverInfo.nanoSeconds / 1000000.0), (long)(asioDriverInfo.samples - last_samples));
...
//{
	...
	//hInst = LoadLibraryA("asiouac2.dll");//动态加载Dll

	//AddProc Add = (AddProc)GetProcAddress(hInst, "DllRegisterServer");//获取Dll的导出函数
	//Add();
	//AddProc Add = (AddProc)GetProcAddress(hInst, "DllUnregisterServer");//获取Dll的导出函数
	//Add();
	//DllRegisterServer();
