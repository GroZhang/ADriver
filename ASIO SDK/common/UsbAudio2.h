...
*/
#ifndef _USBAUDIO2_H_
#define _USBAUDIO2_H_
#pragma pack(1) 
/***********************************************************************/
...
typedef struct
{
	...
	UCHAR bClockID;
	UCHAR bNrPins;
	UCHAR baCSourceId[1];  //TODO
	UCHAR bmControl;                /* bmControls
											* D[1:0] : Clock Selector Control
											* D[7:4] : Reserved (0) */
	UCHAR iClockSelector;

}  USB_Descriptor_Audio_ClockSelector_t;
...
typedef struct
{
	...
	UCHAR bDescriptorType;
	UCHAR bDescriptorSubtype;
	UCHAR bFormatType;            /* Format of the audio stream, see Audio Device Formats specification */
	UCHAR bSubslotSize;           /* Number of bytes for each channels subslot */
	UCHAR bBitResolution;         /* Number of bits used in the above slot for sample  */
}  USB_Descriptor_Audio_Format_Type1_t;
...
#endif
} USB_CfgDesc_Audio2_CS_Control_Int;
...
typedef struct
{
	...
	/* Audio Control */
	USB_INTERFACE_ASSOCIATION_DESCRIPTOR      Audio_InterfaceAssociation;
	USB_INTERFACE_DESCRIPTOR                  Audio_StdControlInterface;       /* Standard Audio Control Interface Header Descriptor */
	USB_CfgDesc_Audio2_CS_Control_Int           Audio_CS_Control_Int;
#ifdef OUTPUT
	/* Audio streaming: Output stream */
	USB_INTERFACE_DESCRIPTOR                  Audio_Out_StreamInterface_Alt0;  /* Zero bandwith alternative */
	USB_INTERFACE_DESCRIPTOR                  Audio_Out_StreamInterface_Alt1;
	USB_Descriptor_Audio_Interface_AS_t         Audio_Out_ClassStreamInterface;
	USB_Descriptor_Audio_Format_Type1_t         Audio_Out_Format;
	USB_ENDPOINT_DESCRIPTOR                   Audio_Out_Endpoint;
	USB_Descriptor_Audio_Class_AS_Endpoint_t    Audio_Out_ClassEndpoint;
	USB_ENDPOINT_DESCRIPTOR                   Audio_Out_Fb_Endpoint;
#if (OUTPUT_FORMAT_COUNT > 1)
	USB_INTERFACE_DESCRIPTOR                  Audio_Out_StreamInterface_Alt2;
	...
	USB_ENDPOINT_DESCRIPTOR                   Audio_Out_Endpoint_2;
	USB_Descriptor_Audio_Class_AS_Endpoint_t    Audio_Out_ClassEndpoint_2;
	USB_ENDPOINT_DESCRIPTOR                   Audio_Out_Fb_Endpoint_2;
#endif
#if (OUTPUT_FORMAT_COUNT > 2)
	USB_INTERFACE_DESCRIPTOR                  Audio_Out_StreamInterface_Alt3;
	...
	USB_ENDPOINT_DESCRIPTOR                   Audio_Out_Endpoint_3;
	USB_Descriptor_Audio_Class_AS_Endpoint_t    Audio_Out_ClassEndpoint_3;
	USB_ENDPOINT_DESCRIPTOR                   Audio_Out_Fb_Endpoint_3;
#endif
#endif
#ifdef INPUT
	/* Audio Streaming: Input stream */
	USB_INTERFACE_DESCRIPTOR                  Audio_In_StreamInterface_Alt0;  /* Zero bandwith alternative */
	USB_INTERFACE_DESCRIPTOR                  Audio_In_StreamInterface_Alt1;
	USB_Descriptor_Audio_Interface_AS_t         Audio_In_ClassStreamInterface;
	USB_Descriptor_Audio_Format_Type1_t         Audio_In_Format;
	USB_ENDPOINT_DESCRIPTOR                   Audio_In_Endpoint;
	USB_Descriptor_Audio_Class_AS_Endpoint_t    Audio_In_ClassEndpoint;
#endif

#ifdef MIDI
	/* MIDI descriptors currently handled as a single block */
	UCHAR configDesc_Midi[MIDI_LENGTH];
#endif

#ifdef DFU
	/* DFU descriptors currently handled as a single block */
	UCHAR configDesc_DFU[DFU_LENGTH];
#endif

#ifdef IAP
	USB_INTERFACE_DESCRIPTOR                  iAP_Interface;
	USB_ENDPOINT_DESCRIPTOR                   iAP_Out_Endpoint;
	USB_ENDPOINT_DESCRIPTOR                   iAP_In_Endpoint;
#ifdef IAP_INT_EP
	USB_ENDPOINT_DESCRIPTOR                   iAP_Interrupt_Endpoint;
#endif
#endif

#ifdef HID_CONTROLS
	USB_INTERFACE_DESCRIPTOR                  HID_Interface;
	UCHAR hidDesc[9];                   //TODO ideally we would have a struct for this.
	USB_ENDPOINT_DESCRIPTOR                   HID_In_Endpoint;
#endif

}USB_Config_Descriptor_Audio2_t;
